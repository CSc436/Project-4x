...
public class Merchant extends Agent {
	...
	public Merchant(UUID id, int playerId, UnitStats new_stats, float xco,
			float yco, int price, int number, int currency) {
		super(id, playerId, BaseStatsEnum.MERCHANT, new_stats,
				GameObjectType.UNIT, UnitType.MERCHANT, xco, yco);
	...
	public void buy(Goods goods, int number, Player p) {
		if (this.getPrestige(p) <= 50)
			...
			int g = p.resources.getGold();
			p.goodsNumber.put(goods, p.getGoodsNumber(goods) + number);
			p.resources.setGold(g - number * goods.price);
		}
	}
}
