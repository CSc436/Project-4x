...
public class Mesh // implements Renderable {
{
	...
	/**
	 * Updates the model matrix of the mesh which governs translation, rotation,
	 * and scale.
	 */
	private void updateModelMatrix() {
		// FloatMatrix rotMatrix =
		// FloatMatrix.createRotationMatrix(rotX,rotY,rotZ);
		// modelMatrix =
		// FloatMatrix.multiply(FloatMatrix.translationMatrix(posX,posY,posZ),FloatMatrix.flipXZMatrix());
		// modelMatrix =
		// FloatMatrix.multiply(FloatMatrix.translationMatrix(posX,posY,posZ),rotMatrix);
	...
	/**
	 * This function constructs the array that enumerates the triangles to be
	 * rendered. Because of how the OBJImporter works, each triangle has its own
	 * set of vertices rather than sharing them between adjacent triangles
	 * (which would be more optimized but who wants to write an optimizer?). The
	 * function just holds [0,1,2,3,4,5,...,(3n-1)].
	 * 
	 * @return
	 */
	...
	/**
	 * Initializes the attribute buffers for the mesh. This holds information
	 * regarding vertices, texture coordinates, colors, normals, and triangles.
	 * 
	 * @param glContext
	 * @return
	 */
	private int initBuffers(WebGLRenderingContext glContext) {
		...
		glContext.bufferData(WebGLRenderingContext.ARRAY_BUFFER,
				Float32Array.create(verts), WebGLRenderingContext.STATIC_DRAW);
		...
		// This might prove useful for certain special effects (see
		// simple_outline.fs)
	...
	private void initTexture(WebGLRenderingContext glContext,
		...
		glContext.texImage2D(WebGLRenderingContext.TEXTURE_2D, 0,
				...
				ImageElement.as(getImage(imageResource).getElement()));
	...
	/**
	 * Unlike tinting, team colors only apply to the left half of a texture.
	 * Generally a texture will place team colored parts of an OBJ on the left
	 * side of the texture so their default color is just white. Team colors
	 * will be used to tint these parts and nothing else. The swordsman for
	 * example will have a colored cape.
	 * 
	 * @param r
	 * @param g
	 * @param b
	 */
	public void setTeamColor(float r, float g, float b) {
		teamRed = r;
		teamGreen = g;
		teamBlue = b;
	}
	...
	public void render(WebGLRenderingContext glContext, Shader shader,
		...
		camPosUniform = glContext.getUniformLocation(shader.shaderProgram,
				"camPos");
		glContext.uniform3f(camPosUniform, cam.getX(), cam.getY(), cam.getZ());

		// Set the uniform variable for the camera position
		cameraPosUniform = glContext.getUniformLocation(shader.shaderProgram,
				"cameraPos");
		glContext.uniform3f(cameraPosUniform, cam.getX(), cam.getY(),
				cam.getZ());

		// Set the uniform variable for the vec4 of the color components of the
		// mesh.
		// This is derived from the id given to the mesh. Unclickable meshes
		// will generally
		// have an id of 0.
		...
		int r = ((id / 65535) % 256);
		glContext
				.uniform4f(idUniform, r / 255.0f, g / 255.0f, b / 255.0f, 1.0f);

		// Set the uniform variable for the health percentage
		healthPercentageUniform = glContext.getUniformLocation(
				shader.shaderProgram, "healthPercentage");
		glContext.uniform1f(healthPercentageUniform, healthPercentage);

		// Set the uniform variable for the team
		teamColorUniform = glContext.getUniformLocation(shader.shaderProgram,
				"teamColor");
		glContext.uniform3f(teamColorUniform, teamRed, teamGreen, teamBlue);
		...
		int texcoordAttribute = glContext.getAttribLocation(
				shader.shaderProgram, "vertexTexCoord");
		glContext.enableVertexAttribArray(texcoordAttribute);
		glContext
				.bindBuffer(WebGLRenderingContext.ARRAY_BUFFER, texcoordBuffer);
		glContext.vertexAttribPointer(texcoordAttribute, 3,
				WebGLRenderingContext.FLOAT, false, 0, 0);

		// Enable the vertexNormal attribute in the vertex shader
		int normalAttribute = glContext.getAttribLocation(shader.shaderProgram,
				"vertexNormal");
		...
		// glContext.getAttribLocation(shader.shaderProgram, "vertexColor");
		// glContext.enableVertexAttribArray(vertexColorAttribute);
		// glContext.bindBuffer(WebGLRenderingContext.ARRAY_BUFFER,
		// colorBuffer);
		// glContext.vertexAttribPointer(vertexColorAttribute, 4,
		// WebGLRenderingContext.FLOAT, false, 0, 0);
		...
		// Set the uniform varaible for the model matrix. This contains things
		// like
		// translation, rotation, and scaling.
		WebGLUniformLocation uniformModelMatrix = glContext.getUniformLocation(
				shader.shaderProgram, "modelMatrix");
		glContext.uniformMatrix4fv(uniformModelMatrix, false,
				modelMatrix.rowWiseData());

		// Actually draw the triangle of the mesh
		glContext.bindBuffer(WebGLRenderingContext.ELEMENT_ARRAY_BUFFER,
				triangleBuffer);
		glContext.drawElements(WebGLRenderingContext.TRIANGLES, triCount * 3,
