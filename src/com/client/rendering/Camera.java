...
public class Camera {
	...
	private float camX = GameCanvas.GRID_WIDTH / 2 * -1,
			camY = GameCanvas.GRID_WIDTH * -1, camZ = 20.0f;
	...
	public void makeCameraMatrix() {
		cameraMatrix = FloatMatrix.createCameraMatrix(
				(float) Math.toRadians(yaw), (float) Math.toRadians(pitch),
				(float) Math.toRadians(roll), 45,
				(float) GameCanvas.WIDTH / (float) GameCanvas.HEIGHT, 0.1f,
				1000000f).columnWiseData();
	}
	...
	public float up(float delta) {
		Vector3 temp = new Vector3((float) -Math.tan(Math.toRadians(yaw))
				* getSign(yaw), getSign(yaw), 10 / camZ);
		temp.normalize();
		camX += temp.x * delta;
		camY += temp.y * delta;
