				...
				if (!chatFlag) {
					...
					case KeyCodes.KEY_I:
						theModel.sendCommand(new PlaceUnitCommand(
								UnitType.CANNON, playerID, mouseTile));
						break;
					case KeyCodes.KEY_K:
						// theModel.sendCommand(new PlaceUnitCommand(
						// UnitType.INFANTRY, (int) (8*Math.random()),
						// mouseTile));
						theModel.sendCommand(new PlaceUnitCommand(
								UnitType.INFANTRY, playerID, mouseTile));
						break;
					case KeyCodes.KEY_L:
						// theModel.sendCommand(new PlaceUnitCommand(
						// UnitType.ARCHER, (int) (8*Math.random()),
						// mouseTile));
						theModel.sendCommand(new PlaceUnitCommand(
								UnitType.ARCHER, playerID, mouseTile));
						break;
					case KeyCodes.KEY_H:
						...
						// mouseTile));
						theModel.sendCommand(new ConstructBuildingCommand(
								BuildingType.BARRACKS, playerID, mouseTile));
						break;
					case KeyCodes.KEY_N:
						for (int i : selectedEntities)
							theModel.sendCommand(new BuildingProductionCommand(
									i, UnitType.INFANTRY));
						break;
					case KeyCodes.KEY_B:
			...
			public void onMouseDown(MouseDownEvent event) {
				...
				case NativeEvent.BUTTON_LEFT:
					if (currMode == Mode.BUILDING) {
						...
						BuildingType bt = BuildingType.valueOf($(
								"#building-toolbar").html());
						Console.log(bt.toString());
						theModel.sendCommand(new ConstructBuildingCommand(bt,
								playerID, mouseTile));
		...
		RootPanel.get().addDomHandler(new MouseMoveHandler() {
			...
			public void onMouseMove(MouseMoveEvent event) {
				...
				RootPanel.get("tile-info").getElement()
						.setInnerHTML(mouseTile.toString());
