...
public class ClientModel {
	...
	private void retrieveGame() {
		simpleSimulator.getGame(playerNumber, -1,
				new AsyncCallback<GameModel>() {
	...
	public void beginPlaying() {
		...
		pollTimer = new Timer() {
			...
			public void run() {
				...
				if (debug)
					Console.log("Attempting to send " + commandQueue.size()
							+ " commands to server");

				// Create a copy so we know exactly which commands are being
				// sent
				// This way we don't lose any command that get added when the
				// async method
				...
				simpleSimulator.sendCommands(playerNumber, commandQueue,
						new AsyncCallback<CommandPacket>() {
							@Override
							public void onFailure(Throwable caught) {
								if (debug)
							...
							public void onSuccess(CommandPacket result) {
								...
								// for chat commands, update the gameInterface.
								// ClientModel likely needs reference to
								// gameInterface?
								// Will try
								for (Command c : commandQueueCopy) {
									// Remove every item in the copy from
									// commandQueue
								...
								if (result == null) {
									...
									if (debug)
										Console.log(result.getCommandQueue()
												.size()
												+ " commands received, simulate for "
												+ result.getTime() + " ms");
									...
									if (debug)
										Console.log("    Simulation state received! Cycle time: "
												+ cycleTime + " ms");
									confirmReceipt();
								}
								resetTimer();
	...
	/**
	 * Resets the Timer in the beginPlaying method This method is called when
	 * the SimpleSimulator Async method 'returns' (callsback?) Also if
	 * 'readyForNext' is false
	 */
	...
	public void confirmReceipt() {
		simpleSimulator.confirmReceipt(playerNumber, turnNumber,
