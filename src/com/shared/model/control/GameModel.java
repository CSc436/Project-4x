...
public class GameModel implements Serializable {
	...
	/**
	 * if we don't actually initialize a list of players anymore, might as well
	...
	// @deprecated
	/*
	 * public GameModel(ArrayList<Player> players, int width) { //this.players =
	 * players;
	 * 
	 * gameObjects = new HashMap<Integer, GameObject>(); attackers = new
	 * HashMap<Integer, Attacker>(); movables = new HashMap<Integer, Movable>();
	 * attackables = new HashMap<Integer, Attackable>(); producers = new
	 * HashMap<Integer, Producer>(); resourceGenerators = new HashMap<Integer,
	 * ResourceGenerator>();
	 * 
	 * map = new GameBoard(width, width); }
	 */
	...
	public Player getPlayer(int playerId) {
		return players.get((playerId + 1)); // Client model is one off from
											// gameModel, so add one....
	}
	 ...
	 * 
	 * @param timeStep
	 *            - number of milliseconds to advance the simulation
	 */
	...
	private void placeNewObjects() {
		...
		for (int k : keySet) {
			...
			/*
			 * int dx = 0, dy = 0; while( !unitQueue.isEmpty() ) {
			 * 
			 * 
			 * if(dy == 0) { dy = -(dx + 1); dx = 0; if(dy > 0) for(int i = 0; i
			 * < dx + dy; i++) {
			 * 
			 * } else for(int i = 0; i < dx + dy; i++) {
			 * 
			 * } } else { dx = dy; dy = 0; }
			 * 
			 * if(dy == 0) for(int i = 0; i < dx + dy; i++) {
			 * 
			 * } if() x += dx; y += dy;
			 * 
			 * }
			...
			for (UnitType ut : unitQueue) {
				Unit u = Factory.buildUnit(getPlayer(playerID), playerID, ut,
						x, y + 1);
				int unitID = u.getId();
		...
		for (GameObject o : producedBuildings) {
			...
			if (o instanceof Producer)
				producers.put(id, (Producer) o);
			if (o instanceof ResourceGenerator)
				resourceGenerators.put(id, (ResourceGenerator) o);
		}
	...
	public ArrayList<GameObject> getVisibleUnits(GameObject viewingObject) {
		...
		int baseX = GameBoard.getCoordEquivalent((float) viewingObject
				.getPosition().getX());
		int baseY = GameBoard.getCoordEquivalent((float) viewingObject
				.getPosition().getY());
		...
		for (Player p : players.values()) {
			Map<Integer, GameObject> goMap = p.getGameObjects()
					.getGameObjects();
			for (Integer u : goMap.keySet()) {
				GameObject go = goMap.get(u);
				int goX = GameBoard.getCoordEquivalent((float) go.getPosition()
						.getX());
				int goY = GameBoard.getCoordEquivalent((float) go.getPosition()
						.getY());
		...
		for (GameObject go : inSquare) {
			double dist = go.getPosition().sub(viewingObject.getPosition())
					.magnitude();
